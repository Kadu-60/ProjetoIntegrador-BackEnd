...
public class ProdutoRepositoryImpl implements ProdutoRepositoryCustom {
    ...
    public List<CardProdutoDTO> findCardsProdutoByBusca(String busca) {
        ...
        Query q =entityManager.createNativeQuery("select pv.id_produto, p.foto, p.nome_produto, p.descricao, pv.valor_preco, P.data_de_criacao, p.destaque from produto p inner join preco_venda pv on (p.id_produto =pv.id_produto) WHERE p.NOME_PRODUTO LIKE '%"+busca+"%' order by pv.data_vigencia desc, pv.id_produto asc","CardProdutoDTO");
        List<CardProdutoDTO> listCard = q.getResultList();
        return listCard;
    }
